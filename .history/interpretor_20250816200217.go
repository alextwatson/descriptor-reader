...
func Explain(n *Node) string {
	...
	case NodeTr:
		if len(n.Children) == 1 {
			return fmt.Sprintf(
				"Taproot key path spend (single option):\n• %s",
				Explain(n.Children[0]),
			)
		}

		// At-a-glance header
		summary := fmt.Sprintf("Taproot script path spend (%d options):", len(n.Children))
	...
	default:
		return fmt.Sprintf("unrecognized miniscript: %s", n.Value)
