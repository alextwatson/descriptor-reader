...
type Node struct {
	Type     NodeType
	Value    string  // for numbers, keys
	Children []*Node // for subexpressions
}

func main() {
	tests := []string{
		"and_v(pk(alice),older(12960))",
		"or_d(pk(bob),and_v(pk(carol),after(500000)))",
...
func Explain(n *Node) string {
	...
	case NodeOlder:
		return fmt.Sprintf("after %s blocks confirmation", n.Value)
	case NodeMulti:
...
// helper: get string inside parentheses
...
// splitArgs handles commas at correct depth
